<template>
  <header class="nav center">
    ...
    <router-link class="nav-ul_li nav_link" :class="{ 'nav_link__active': $route.path === '/' }" to="/">Главная</router-link>
    ...
    <div v-else>
      <router-link class="nav-ul_li nav_link" :class="{ 'nav_link__active': $route.path === '/our_team' }" to="/our_team">Мастера</router-link>
    </div>
      <div class="page-loader" v-if="loading">
        <div class="loader">
          <div class="loader-diamond"></div>
      ...
      <div v-else>
      <router-link class="nav-ul_li nav_link" :class="{ 'nav_link__active': $route.path === '/price_for_services' }" to="/price_for_services">Косметика</router-link>
    </div>
  ...
  <div v-else> 
  <router-link class="nav-ul_li nav_link" :class="{ 'nav_link__active': $route.path === '/contact' }" to="/contact">Контакты</router-link>
  </div>
...
<hr>
  ...
  <router-view name="OurWork"></router-view>
  <router-view name="OurTeam"></router-view>
  <router-view name="Contacts"></router-view>
  <router-view name="PriceForServices"></router-view>
...
export default {
  data() {
    return {
      loading: true // Инициализируем состояние загрузки
    };
  },
  mounted() {
    // Имитируем задержку загрузки страницы
    setTimeout(() => {
      this.loading = false; // Завершили загрузку
    }, 3000); // Пример задержки в 2 секунды, вы можете изменить значение
  }
};
...
.nav-ul_li:hover, .nav_link__active {
  color: #CDAA7D;
...
.loader-diamond {
  ...
  animation: diamond-animation 1s infinite alternate; 
}
